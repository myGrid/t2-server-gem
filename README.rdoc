...
You can delete a run from the server by supplying either a run object or a run
identifier to the <tt>delete_run</tt> method, along with your credentials:

  server.delete_run(run, credentials)

And all runs can be deleted in one go too:
